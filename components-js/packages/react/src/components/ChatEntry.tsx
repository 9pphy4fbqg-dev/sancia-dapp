  ...
  function ChatEntry(
    ...
    const formatUsername = (username: string) => {
      ...
      if (username.startsWith('0x')) {
        // 钱包地址，显示0x+后四位
        return `0x${username.slice(-4)}`;
      }
      // 普通用户名，显示后四位
      return username.slice(-4);
    };
    
    const name = formatUsername(entry.from?.name ?? entry.from?.identity ?? '');
    ...
    return (
      <li
        ...
        {...props}
      >
        <span className="lk-chat-content">
          {/* 用户名 */}
          {!hideName && (
            <strong className="lk-participant-name">{name}</strong>
          )}
